...
def get_question_and_answers(url):
    ...
    if soup == None:  # Captcha page
        ...
        question_stats = soup.find(
            "div", attrs={"itemprop": "upvoteCount"}).get_text()  # Vote count
        question_stats += " Votes | Asked " + \
            soup.find("time", attrs={
                      "itemprop": "dateCreated"}).get_text()  # Date created
        question_desc = stylize_code(soup.find_all(
            "div", class_="s-prose js-post-body")[0])  # TODO: Handle duplicates

        answers = [stylize_code(answer) for answer in soup.find_all(
            "div", class_="s-prose js-post-body")][1:]
        if len(answers) == 0:
...
class ScrollBar(urwid.WidgetDecoration):
    ...
    def render(self, size, focus=False):
        ...
        if thumb_height + top_height + bottom_height == maxrow:
    ...
    def _handle_input(self, input):
        ...
        if input == "enter" or (input[0] == 'meta mouse press' and input[1] == 1):
            ...
            if url != None:
                ...
                self.content[idx] = urwid.AttrMap(
                    SelectableText(txt), 'viewed', 'reveal viewed focus')
