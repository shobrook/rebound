...
def souper(url):
    ...
    # if re.search("\.com/nocaptcha", req.get_full_url()): # URL is a captcha page
    #     return None
    # else:
    ...
    try:
        ...
        resp = urlopen(redirected_url)
        # print('Redirected -> %s' % redirected_url)  # the original redirected url 
    # print('Response URL -> %s ' % resp.url)  # the final url/

    if re.search("\.com/nocaptcha", resp.url): # URL is a captcha page
        print('captcha check! open url in browser to whitelist')
        print('Response URL -> %s ' % resp.url)  # the final url/
