...
def souper(url):
    ...
    if re.search("\.com/nocaptcha", html.url): # Return None if the URL is a captcha page
        # IDEA: Solve the captcha with Selenium. The checkbox is in an iframe, so (theoretically)
        # we could target the captcha container, switch into the iframe, target the checkbox and
        # call .click(), and then switch back to SO
        return None
    else:
    ...
    def __handle_input(self, input):
        if input == "enter": # View answers
            ...
            if url != None:
                ...
                pile = urwid.Pile(self.__stylize_question(question_title, question_desc, question_stats) + [urwid.Divider('-')] + [urwid.Divider('-')] +
                interleave(list(map(lambda answer: self.__stylize_answer(answer), answers)), [urwid.Divider('-')] * (len(answers) - 1)))
